...
export class AuthenticationService {
  ...
  login(context: LoginContext): Observable<any> {    
    ...
    return this.http.post("login", context);
  }
  ...
  logout(): Observable<any> {
    ...
    if (savedCredentials) {
      let context = { token_type_hint: "refresh_token", token : JSON.parse(savedCredentials).accessToken };
  ...
  refreshToken(): Observable<any> {
    ...
    let context = { grant_type: "refresh_token", refresh_token: JSON.parse(storage.getItem('credentials')).refreshToken };
