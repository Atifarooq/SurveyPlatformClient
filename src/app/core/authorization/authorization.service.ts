...
export class AuthorizationService {
  ...
  constructor(
    ...
    if(this.authenticationService.credentials && this.authenticationService.credentials.user)
      this.getPermissions();
  }

  async getPermissions(): Promise<any> {
   
    let perm = await this.http.get(`${this.userEndpoint}/${this.authenticationService.credentials.user.user_id}/role`).toPromise();
